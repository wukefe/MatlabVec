...
solutions = sum(nqueen_solver1(size, 3:size))

unique_solutions = solutions;
solutions = solutions * 8;

for i = 1:floor(size / 2) - 1
    [a_solutions,u_solutions] = nqueen_solver(size, bitshift(1,size)-1,  bitshift(1,i), bitshift(1,i+1), bitshift(bitshift(1,i),-1));
